...
/**
 * 紧凑 Scope Summary（RWV2-13 Issue #34，RFC §3 唯一 Scope 契约）。
 *
 * 完整 Sources/Notes 选择编辑面在左栏（唯一编辑面）；摘要只保留：
 * - 模式 + 计数摘要（entire_project 显式可见，D2）——常驻展示，不折叠；
 * - `Edit scope` 入口：经组合层退出最大化并回到左栏编辑面，**不持有**任何
 *   第二套选择/模式状态（不变量 1：一个 Scope 真源）；
 * - 加载/失败/重试状态沿用原选择器语义。
 *
 * RWV2-UIOPT-A（fork #57）：组件迁入 Header 左侧；自身不再携带
 * `border-b px-3 py-2`（Header 统一负责唯一视觉边界）；长 Scope 文本视觉
 * 截断，完整语义经 `title` 保留。
 * 摘要文本以 `aria-live=polite` 播报：左栏复选框写入 provider 后，摘要
 * 与所有消费方立即同步（AC：左栏选择即时更新摘要与每次动作）。
 */
