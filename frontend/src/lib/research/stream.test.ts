...
// 注：与 UI-02 合并后本套件独立为 stream.test.ts（UI-02 的 api.test.ts
// 以 axios adapter 覆盖 Sources/Notes/Transformations/Export 端点契约；
// 本套件覆盖 SSE 流/重连/Gateway 配置，避免模块级 mock 相互冲突）。
