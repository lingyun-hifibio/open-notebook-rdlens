...
### Added
- **Research (RWV2-11, #32): Search/Chat/Coverage/Compare dispatch on an immutable scope snapshot.** Every research action now freezes `ResearchScopeProvider.getSnapshot()` at click time and derives payload, idempotency input, egress-consent summary and visible turn metadata from that one frozen snapshot — changing the scope mid-flight never alters an in-flight turn or job. Chat retry re-uses the failed turn's original snapshot instead of the current selection; restored turns (reload) truthfully keep no scope and no retry button, so a "retry" can never silently widen to the whole project. The egress-consent dialog shows the dispatched scope (e.g. "Entire project" / "2 sources · 1 note") only for dispatches that registered one. Compare and Coverage are now mode-aware: `Entire project` has no explicit source set, so both show a clear English message and stay disabled until RWV2-14 pagination lands (Compare remains Source-only; Coverage still rejects empty source lists server-side); Search's `document` context converges to `focused` with a message in `Entire project`. Panels consume the shared scope directly (`selectedSourceIds`/`selectedNoteIds` props removed); no RDLens/API/DB change is required. See [ADR-008](docs/7-DEVELOPMENT/decisions/ADR-008-research-scope-snapshot-dispatch.md).
